...
from video_timestamps import best_source
...
def test_get_pts_mkv() -> None:
    ...
    pts_list, time_base, fps = best_source.get_pts(str(video_file_path), 0)
...
def test_get_pts_non_video_index() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        best_source.get_pts(str(video_file_path), 1)
...
def test_get_pts_invalid_index() -> None:
    ...
    with pytest.raises(ValueError) as exc_info:
        best_source.get_pts(str(video_file_path), 2)
